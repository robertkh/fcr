	...
	try {
		...
		if (!user.isReged) {
			return (
				res
					.status(500)
					//! Շատ հետաքրքիր է, որ եթե սա սխալ է (send()), ապա նախորդ պատասխանն է ուղարկում։ Սա լավ շփոթ է առաջացնում։
					.json(
						"Ձեր հաշիվը ակտիվ չէ։ Ստուգեք ձեր էլեկտրոնային հասցեն։ "
					)
			);
		}

		// user.id -> string; user._id -> object
		if (user.isAdmin) {
			const token = user.generateAuthToken();
			//res.set("isAdmin", true);res.set("name", user.username);
